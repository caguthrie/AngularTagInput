        ...
        link: function(scope, element, attrs, ngModelCtrl) {
            ...
            input
                .on('keydown', function(e) {
                    ...
                    shouldRemove = !shouldAdd && key === KEYS.backspace && scope.newTag.text.length === 0 && !scope.disableBackspace;
...
tagsInput.run(["$templateCache", function($templateCache) {
    ...
    "<div class=\"host\" tabindex=\"-1\" ti-transclude-append=\"\"><div class=\"tags\" ng-class=\"{focused: hasFocus}\"><ul class=\"tag-list\"><li class=\"tag-item\" ng-repeat=\"tag in tagList.items track by track(tag)\" ng-class=\"{ selected: tag == tagList.selected }\"><span>{{getDisplayText(tag)}}</span> <a class=\"remove-button\" ng-hide=\"dontRemoveFirst && $index<dontRemoveFirst\" ng-click=\"tagList.remove($index)\">{{options.removeTagSymbol}}</a></li></ul><input class=\"input form-control\" placeholder=\"{{options.placeholder}}\" tabindex=\"{{options.tabindex}}\" ng-model=\"newTag.text\" ng-change=\"newTagChange()\" ng-trim=\"false\" ng-class=\"{'invalid-tag': newTag.invalid}\" ti-autosize=\"\"></div></div>"
  );
